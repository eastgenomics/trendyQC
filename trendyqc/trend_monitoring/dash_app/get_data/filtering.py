...
def get_subset_queryset(data: Dict) -> QuerySet:
    ...
    if date_start and date_end:
        ...
        if days_back:
            # calculate the date range at the filtering level in order to keep
            # the days back option dynamic i.e. if a filter is saved with 30
            # days back and is used at the beginning of the month or at the
            # end of the month, the results will be different
        ...
        if len(metrics) > 1:
            ...
            trace_args = {
                **{
                    ...
                    "boxplot_line_color": darken_hex(seen_groups[legend_name]),
                    "showlegend": shown_legend,
                },
                **legend_args,
            }
...
def create_trace(**kwargs):
    ...
    trace = {
        "x": [kwargs["project_name"]] * len(data_values),
        "y": data_values,
        "name": kwargs["name"],
        "type": "box",
        ...
        "line": {"color": kwargs.get("boxplot_line_color", "#000000")},
        "fillcolor": kwargs["boxplot_color"],
        "opacity": 0.9,
        # grouping of boxes
